...
void lpNormDeviceThreadFunc(LPNormThreadTestParams<T> &params) {
    try {
        ...
        auto op = g->addOp<LPNormObj>(x, nullptr, params.axis, params.p,
                                       params.eps);
...
void runLPNormMultiThreadTest(const Shape &shapeIn, int axis, int p,
    ...
    EXPECT_EQ(numErrors, 0)
        << "CPU vs NVIDIA mismatch for LPNorm (p=" << p
        << " axis=" << axis << ", max error: " << maxError << ")";
}
...
TEST(LPNorm, L2_Axis1_MultiThread_F32) {
    runLPNormMultiThreadTest<float>({8, 32}, 1, 2,
                                    DataType(INFINI_DTYPE_F32), 1e-4f);
}

TEST(LPNorm, L1_Axis0_MultiThread_F32) {
    runLPNormMultiThreadTest<float>({16, 16}, 0, 1,
                                    DataType(INFINI_DTYPE_F32), 1e-4f);
}
