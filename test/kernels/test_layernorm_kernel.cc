...
void layerNormDeviceThreadFunc(LayerNormThreadTestParams<T> &params) {
    try {
        ...
        auto op    = g->addOp<LayerNormObj>(x, w, b, nullptr,
                                            params.axis, params.eps);
...
TEST(LayerNorm, MultiThread_2D_F32) {
    runLayerNormMultiThreadTest<float>({2, 8, 32}, {32}, -1,
                                       1e-5f, DataType(INFINI_DTYPE_F32));
}

TEST(LayerNorm, MultiThread_3D_F32) {
    runLayerNormMultiThreadTest<float>({2, 4, 16}, {16}, -1,
                                       1e-5f, DataType(INFINI_DTYPE_F32));
}
#else
